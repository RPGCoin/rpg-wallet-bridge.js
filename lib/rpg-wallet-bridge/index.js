...
const rpgaddrjs_1 = require("rpgaddrjs");
const rpgcoinjs = require("rpgcoinjs");
const Utxo_1 = require("rpg-wallet-bridge-provider-interface/lib/entities/Utxo");
class RPGWalletBridge {
    constructor(walletProvider) {
        ...
        this.isP2SHLegacyAddress = (address) => {
            try {
                if (!this.isLegacyAddress(address) || !rpgaddrjs_1.isP2SHAddress(address)) {
                    return false;
                }
            }
            catch (e) {
        ...
        this.isLegacyAddress = (address) => {
            try {
                if (!rpgaddrjs_1.isLegacyAddress(address)) {
                    return false;
                }
            }
            catch (e) {
        ...
        this.toAddressFromScript = (script) => {
            ...
            const hashed = rpgcoinjs.crypto.Hash.sha256ripemd160(buf);
            const legacy = rpgcoinjs.Address.fromScriptHash(hashed).toString();
            return rpgaddrjs_1.toLegacyAddress(legacy);
        };
    }
    ...
    async sign(address, dataToSign) {
        if (!this.isLegacyAddress(address)) {
            throw new IllegalArgumentException_1.default("The address is not RPG Address format.");
        }
...
exports.default = RPGWalletBridge;
