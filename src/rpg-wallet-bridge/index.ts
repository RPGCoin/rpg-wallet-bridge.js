...
import { isLegacyAddress, isP2SHAddress, toLegacyAddress} from "rpgaddrjs"
import * as rpgcoinjs from "rpgcoinjs-lib"
...
export default class RPGWalletBridge implements IRPGWalletBridge {
  ...
  public async getRedeemScript(
    ...
    if (!this.isP2SHLegacyAddress(p2shAddress)) {
      throw new IllegalArgumentException("The address is not P2SH Address or Rpg Address.")
    }
  ...
  public async sign(
    ...
    if (!this.isLegacyAddress(address)) {
      throw new IllegalArgumentException("The address is not RPG Address format.")
    }
